...
TEST_CASE("apply_all", "[registry]")
{
    ...
    auto logger = std::make_shared<spdlog::logger>(tested_logger_name, std::make_shared<spdlog::sinks::null_sink_st>());
    spdlog::register_logger(logger);
    auto logger2 = std::make_shared<spdlog::logger>(tested_logger_name2,
...
TEST_CASE("disable automatic registration", "[registry]")
{
    ...
    auto logger1 = spdlog::create<spdlog::sinks::daily_file_sink_st>(tested_logger_name, SPDLOG_FILENAME_T("filename"),
                                                                     11, 59);
